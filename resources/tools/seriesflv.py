    ...
    for entry in chapters:
        if title.find("Subtitulada") >= 0:            
            if entry.find('lang="sub"') >=0:
                ...
                plugintools.add_item(action="chapter_urls", title='[COLOR orange]'+num_chapter+'[/COLOR]'+'  [COLOR lightyellow][B]'+title_chapter+'[/B][/COLOR][COLOR lightgreen][I] ('+i_time+')[/I][/COLOR]' , info_labels = datamovie , extra = show , page = show , url = url_chapter , thumbnail = thumbnail , fanart = fanart , folder = True, isPlayable = False)
        ...
        if title.find("Español") >= 0:        
            if entry.find('lang="es"') >= 0:
                ...
                i_time = plugintools.find_single_match(entry, '<div class="i-time">(.*?)</div>')
                #plugintools.log("desde hace= "+i_time)
                plugintools.add_item(action="chapter_urls", title='[COLOR orange]'+num_chapter+'[/COLOR]'+'  [COLOR lightyellow][B]'+title_chapter+'[/B][/COLOR][COLOR lightgreen][I] ('+i_time+')[/I][/COLOR]' , url = url_chapter , info_labels = datamovie , extra = show , page = show , thumbnail = thumbnail , fanart = fanart , folder = True, isPlayable = False)
...
def chapter_urls(params):
    ...
    plugintools.add_item(action="", title='[COLOR orange]'+title+'[/COLOR]' , url = "" , info_labels = datamovie , extra = show , page = show , thumbnail = thumbnail , fanart = fanart , folder = True, isPlayable = False)
        ...
        if len(server_name) >= 2:
            ...
            if url != "":
                if url.find("allmyvideos") >= 0:
                    ...
                    plugintools.add_item(action="mailru", title= '[COLOR white]'+title_fixed+' [/COLOR][COLOR lightyellow][I]('+server_name+')[/I][/COLOR]  [COLOR lightgreen][I]'+lang+'[/I][/COLOR]' , url = url, info_labels = datamovie , extra = show , page = show , fanart = fanart , thumbnail = thumbnail , folder = False, isPlayable = True)
